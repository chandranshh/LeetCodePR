class Solution {
public:
    int helper(int idx, int end, bool canBuy, vector<int>& prices, vector<vector<int>>& dp) {
        if (idx == end) {
            return 0;
        }
        if (dp[idx][canBuy] != -1) {
        ...
        int profit = 0;
        if (canBuy) {
            int buy = -prices[idx] + helper(idx + 1, end, false, prices, dp);
            int noBuy = helper(idx + 1, end, true, prices, dp);
            profit = max(buy, noBuy);
        } else {
            int sell = prices[idx] + helper(idx + 1, end, true, prices, dp);
